...
function updateFromLockedBalances() {
  const lockedBalancesData = JSON.parse(
    fs.readFileSync(path.join(INPUT_JSON_DIR, "locked_balances.json"))
  );
  lockedBalancesData.forEach((entry) => {
    ...
    if (JSBI.greaterThan(entry.totalERC20, JSBI.BigInt(0))) {
      results[checksumAddress]["locked_balances.json"] = JSBI.BigInt(
        entry.totalERC20
      ).toString();
      results[checksumAddress].balance = JSBI.add(
        JSBI.BigInt(results[checksumAddress].balance),
        JSBI.BigInt(entry.totalERC20)
      ).toString();
    }
  });
}
