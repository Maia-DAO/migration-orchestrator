    ...
    for (let i = 0; i < tokenList.length; i++) {
        ...
        stakedData.forEach(gauge => {
            gauge.stakes.forEach(stake => {
                ...
                if (addr.trim().toLowerCase() === strategyAddress.trim().toLowerCase()) {
        ...
        await processTokenBalancesAndShares(Multicall, accounts, vaultAddress, balance, i);
    }
}
...
async function processCSV(filename) {
    ...
    Object.keys(results).forEach(addr => {
        outputData.push({ address: addr, balance: results[addr].balance.toString() });
    });
...
async function processTokenBalancesAndShares(Multicall, accounts, vaultAddress, balance, i) {
    ...
    const totalSupplyCall = [({
        target: vaultAddress,
        callData: new ethers.utils.Interface(MOO_ABI).encodeFunctionData("totalSupply", [])
    })];
    ...
    await processAndFormatResults(vaultAddress, accounts, returnData, balanceCalls.length, balance, i);
}

async function processAndFormatResults(vaultAddress, accounts, returnData, balanceCallsLength, balance, i) {
    const results = [];
...
function mergeData(oldCsv, stakes, divisor) {
    ...
    Array.from(csvMap, ([address, balance]) => ({
        address,
        balance
    })).sort((a, b) => b.balance - a.balance).forEach(({ address, balance }) => outputData += `${address},${balance}\n`)
