...
const PROVIDER_URL = "https://metis-mainnet.g.alchemy.com/v2/FWmhvca-2KGl6D1o9YcToyEeO8Lmshcy";
const MIGRATION_BLOCK = 18011710;
const MULTICALL_ADDRESS = "0x5D78bF8f79A66e43f5932c1Ae0b8fA6563F97f74";
const BATCH_SIZE = 100;
const TOKEN_LIST = [
    { address: "0x72c232D56542Ba082592DEE7C77b1C6CFA758BCD", creationBlock: 181136, outputFile: "./consolidated_output/maia_new.csv" },
    { address: "0xb27BbeaACA2C00d6258C3118BAB6b5B6975161c8", creationBlock: 1324876, outputFile: "./consolidated_output/hermes_new.csv" },
    { address: "0xaFF73f55968Ab4b276a26E574c96e09A615b13d6", creationBlock: 1283785, outputFile: "./consolidated_output/starHermes_new.csv" },
    { address: "0xD7a586CE5250bEfaB2cc2239F7226B9602536E6A", creationBlock: 3522043, outputFile: "./consolidated_output/sMaia_new.csv" },
    { address: "0x559119275a8A862EdbD2cDE21196f4c758D5Ab84", creationBlock: 200712, outputFile: "./consolidated_output/old_sMaia_new.csv" },
];
...
async function processToken({ address, creationBlock, outputFile }) {
    try {
        ...
        console.log(`Processing token: ${address} from block ${creationBlock} to ${block}`);
        ...
        for (let i = 0; i < accounts.length; i += BATCH_SIZE) {
            ...
            console.log(`Processing batch ${i / BATCH_SIZE + 1} with ${batchAccounts.length} accounts`);
            await processTokenBalancesAndShares(Multicall, batchAccounts, address, outputFile);
        }
    } catch (error) {
...
async function fetchLogsInBatches(vault, startBlock, endBlock, batchSize) {
    ...
    for (let currentBlock = startBlock; currentBlock <= endBlock; currentBlock += batchSize) {
        ...
        try {
            ...
            const logs = await vault.queryFilter({ topics: [transferEventSignature] }, currentBlock, batchEndBlock);
            allLogs = allLogs.concat(logs);
        } catch (error) {
            console.error(`Error fetching logs from ${currentBlock} to ${batchEndBlock}:`, error);
            break;
        }
    }
    return allLogs;
...
async function processTokenBalancesAndShares(Multicall, accounts, vaultAddress, outputFile) {
    try {
        const balanceCalls = accounts.map(account => ({
            target: vaultAddress,
            callData: new ethers.utils.Interface(MOO_ABI).encodeFunctionData("balanceOf", [account])
        }));
...
async function processAndFormatResults(accounts, returnData, outputFile) {
    ...
    const accountBalances = accounts.map((account, index) => {
        const tokenBalance = ethers.utils.defaultAbiCoder.decode(["uint256"], returnData[index])[0].toString();
        return { account, balance: JSBI.BigInt(tokenBalance) };
    });

    // Sort by balance in descending order
    accountBalances.sort((a, b) => { JSBI.lessThan(JSBI.BigInt(b.balance), JSBI.BigInt(a.balance)) ? -1 : 1 });

    // Format the sorted balances as CSV lines
    const csvLines = accountBalances.map(({ account, balance }) => `${account},${balance}`).join('\n');
